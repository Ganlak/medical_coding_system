...
class LoggingConfig:
    ...
    # Extract filename only from LOG_FILE (remove any path components)
    log_file_env = os.getenv("LOG_FILE", "app.log")
    LOG_FILE = LOGS_DIR / Path(log_file_env).name 
    ...
    # Extract filename only from AUDIT_LOG_FILE
    audit_log_file_env = os.getenv("AUDIT_LOG_FILE", "audit.log")
    AUDIT_LOG_FILE = LOGS_DIR / Path(audit_log_file_env).name  
